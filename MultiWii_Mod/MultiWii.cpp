...
void loop () {
  ...
  int xbeeDat[5];             //JH
  int failcount_temp;         //JH
  get_xbee(&xbeeDat[0]);      //JH
  ...
  failcount_temp= xbeeDat[0];
 
  rcData[ROLL] = xbeeDat[1];
  rcData[YAW] = xbeeDat[2];
  rcData[PITCH] = xbeeDat[3];
  rcData[THROTTLE] = xbeeDat[4];
  #if defined(FAILSAFE)
  if (failcount_temp == 0) {
  if(failsafeCnt > 60) failsafeCnt -= 60; else failsafeCnt = 0;
  }
  #endif
